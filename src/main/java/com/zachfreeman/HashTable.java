...
public class HashTable {
    ...
    // same as get but decrements counter, signifying it being bought
    int buy(Object key, int amount){
        int h = MurmurHash2.hash32(key.toString());
        int i = h & (table.length-1);
        if(table[i] != null){
            if(key.equals(table[i].key)) {
                table[i].val = table[i].val - amount;
                return table[i].val;
            }
            else
                i = LinearProbe(i, key);
    ...
    private int LinearProbe(int i, Object key){    // desired key not found at i. probe linearly until the key is found, but if it does not exist, return null.
        ...
        return index;                               // reaches here when a null slot was hit.
    }
